...
describe("EditorView extension", () => {
  it("calls update when the viewport changes", () => {
    const viewports: {from: number, to: number}[] = []
  ...
  it("allows content attributes to be changed through effects", () => {
    const cm = tempView("", [EditorView.contentAttributes.of({spellcheck: "true"})])
    ist(cm.contentDOM.spellcheck, true)
  })

  it("allows editor attributes to be changed through effects", () => {
    const cm = tempView("", [EditorView.editorAttributes.of({class: "something"})])
  ...
  it("redraws the view when phrases change", () => {
    const plugin = ViewPlugin.fromClass(class {
      ...
      constructor(view: EditorView) {
        const elt = this.elt = view.dom.appendChild(document.createElement("div"))
    ...
    const cm = tempView("one", [plugin, lang.of([])])
