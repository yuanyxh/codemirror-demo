...
describe("EditorView selection", () => {
  it("can read the DOM selection", () => {
    ...
    const one = textNode(cm.contentDOM, "one")!
    const three = textNode(cm.contentDOM, "three")!
    test(one, 0, 0)
    test(one, 1, 1)
    test(one, 3, 3)
  ...
  it("syncs the DOM selection with the editor selection", () => {
    const cm = requireFocus(tempView("abc\n\ndef"))
    ...
    const abc = textNode(cm.contentDOM, "abc")!
    const def = textNode(cm.contentDOM, "def")!
