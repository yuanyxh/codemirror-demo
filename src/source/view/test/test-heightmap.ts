...
describe("HeightMap", () => {
  ...
  function doc(... lineLen: number[]) {
    const text = lineLen.map(len => "x".repeat(len))
    return Text.of(text)
  }
  ...
  it("stores information about block widgets", () => {
    const text = doc(3, 3, 3), oracle = o(text)
    ...
    const {type} = map.lineAt(0, byP, oracle, 0, 0)
  ...
  it("materializes lines for measured heights", () => {
    const text = doc(10, 10, 10, 10), oracle = o(text)
    const map = mk(text, [])
  ...
  it("doesn't set the heightChanged flag for small within-line replacements", () => {
    const text = doc(10, 10, 10), oracle = o(text)
    let map = mk(text, []).updateHeight(oracle, 0, false, new MeasuredHeights(0, [12, 12, 12]))
    const newText = text.replace(21, 21, Text.of(["!"]))
  ...
  it("can update lines across the tree", () => {
    const text = doc(...new Array(100).fill(10)), oracle = o(text)
  ...
  it("can handle inserting a line break", () => {
    ...
    ist(map.size, 3)
    const text2 = text.replace(3, 3, Text.of(["", ""]))
  ...
  it("can handle insertion in the middle of a line", () => {
    const text = doc(3, 3, 3), oracle = o(text)
    let map = mk(text).updateHeight(oracle, 0, false, new MeasuredHeights(0, [10, 10, 10]))
    const text2 = text.replace(5, 5, Text.of(["foo", "bar", "baz", "bug"]))
  ...
  describe("blockAt", () => {
    ...
    it("finds widget blocks", () => {
      ...
      const oracle = o(text)
      const block1 = map.blockAt(0, oracle, 0, 0)
      ist(block1.from, 0); ist(block1.to, 3)
      const block2 = map.blockAt(block1.height + 1, oracle, 0, 0)
      ...
      const top3 = block2.bottom + block1.height
      const block3 = map.blockAt(top3 + 10, oracle, 0, 0)
      ist(block3.from, 8); ist(block3.to, 11)
      ist(block3.top, top3); ist(block3.height, 30)
      ist(block3.type, BlockType.WidgetRange)
      const block4 = map.blockAt(block3.bottom + block1.height, oracle, 0, 0)
  ...
  describe("lineAt", () => {
    ...
    it("finds lines in lines", () => {
      const text = doc(3, 3, 3, 3), oracle = o(text)
      const map = mk(text).updateHeight(oracle, 0, false, new MeasuredHeights(0, [10, 10, 20, 10]))
      const line1 = map.lineAt(0, byP, oracle, 0, 0)
      ist(line1.from, 0); ist(line1.to, 3)
      ist(line1.top, 0); ist(line1.bottom, 10)
      ist(map.lineAt(9, byH, oracle, 0, 0), line1, eqBlock)
      const line2 = map.lineAt(9, byP, oracle, 0, 0)
    ...
    it("includes adjacent widgets in lines", () => {
      ...
      const map = mk(text, [Decoration.widget({widget: new MyWidget(100), block: true, side: -1}).range(4),
                          ...
                          Decoration.widget({widget: new MyWidget(0), block: true, side: 1}).range(15)])
      const line1 = map.lineAt(4, byP, oracle, 0, 0)
      ...
      const line2 = map.lineAt(map.height, byH, oracle, 0, 0)
