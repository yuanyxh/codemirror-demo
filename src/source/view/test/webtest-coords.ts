...
describe("EditorView coords", () => {
  ...
  it("can find coordinates in text scrolled into view horizontally", () => {
    const cm = tempView("l1\n" + "l2 ".repeat(400))
    const rect = cm.dom.getBoundingClientRect(), line2 = cm.coordsAtPos(3)!.top + 2
    cm.scrollDOM.scrollLeft = 0
    const right = cm.posAtCoords({x: rect.right - 2, y: line2})
  ...
  it("takes coordinates before side=1 widgets", () => {
    const widget = Decoration.widget({widget: inline, side: 1})
    const cm = tempView("abdefg", [deco(widget.range(0), widget.range(3), widget.range(6))])
    const sides = Array.prototype.map.call(cm.contentDOM.querySelectorAll(".widget"), w => w.getBoundingClientRect().left)
  ...
  it("takes coordinates after side=-1 widgets", () => {
    const widget = Decoration.widget({widget: inline, side: -1})
    const cm = tempView("abdefg", [deco(widget.range(0), widget.range(3), widget.range(6))])
    const sides = Array.prototype.map.call(cm.contentDOM.querySelectorAll(".widget"), w => w.getBoundingClientRect().right)
    ist(near(cm.coordsAtPos(0, 1)!.left, sides[0]))
    ist(near(cm.coordsAtPos(0, -1)!.left, sides[0]))
    ist(near(cm.coordsAtPos(3, 1)!.left, sides[1]))
    ist(near(cm.coordsAtPos(3, -1)!.left, sides[1]))
  ...
  it("respects sides for widgets wrapped in marks", () => {
    ...
    const widgets = cm.contentDOM.querySelectorAll(".widget")
    const pos2 = widgets[0].getBoundingClientRect().left
    ist(near(cm.coordsAtPos(2, 1)!.left, pos2))
    ist(near(cm.coordsAtPos(2, -1)!.left, pos2))
    const pos5 = widgets[1].getBoundingClientRect().right
    ist(near(cm.coordsAtPos(5, 1)!.left, pos5))
    ist(near(cm.coordsAtPos(5, -1)!.left, pos5))
  })

  it("takes coordinates between widgets", () => {
    const wb = Decoration.widget({widget: inline, side: -1})
    const wa = Decoration.widget({widget: inline, side: 1})
    const cm = tempView("abdefg", [deco(wb.range(0), wa.range(0), wb.range(3), wa.range(3), wb.range(6), wa.range(6))])
    const sides = Array.prototype.map.call(cm.contentDOM.querySelectorAll(".widget"), w => w.getBoundingClientRect().right)
    ist(near(cm.coordsAtPos(0, 1)!.left, sides[0]))
    ist(near(cm.coordsAtPos(0, -1)!.left, sides[0]))
  ...
  it("takes coordinates before side=1 block widgets", () => {
    const widget = Decoration.widget({widget: block, side: 1, block: true})
    const cm = tempView("ab", [deco(widget.range(0), widget.range(1), widget.range(2))])
    const sides = Array.from(cm.contentDOM.querySelectorAll(".widget")).map(w => w.getBoundingClientRect().top)
  ...
  it("takes coordinates after side=-1 block widgets", () => {
    const widget = Decoration.widget({widget: block, side: -1, block: true})
    const cm = tempView("ab", [deco(widget.range(0), widget.range(1), widget.range(2))])
    const sides = Array.prototype.map.call(cm.contentDOM.querySelectorAll(".widget"), w => w.getBoundingClientRect().bottom)
  ...
  it("takes coordinates around non-inclusive block widgets", () => {
    const widget = Decoration.replace({widget: block, inclusive: false, block: true})
    const cm = tempView("ab", [deco(widget.range(0, 2))])
    const rect = cm.contentDOM.querySelector(".widget")!.getBoundingClientRect()
  ...
  it("takes proper coordinates for elements on decoration boundaries", () => {
    const cm = tempView("a b c", [deco(Decoration.mark({attributes: {style: "padding: 0 10px"}}).range(2, 3))])
...
describe("coordsForChar", () => {
  ...
  it("returns reasonable coords", () => {
    ...
    const a = cm.coordsForChar(0)!, c = cm.coordsForChar(2)!, d = cm.coordsForChar(4)!, f = cm.coordsForChar(6)!
    ist(a.right, a.left, ">")
    ist(a.bottom, a.top, ">")
  ...
  it("returns proper rectangles in right-to-left text", () => {
    const cm = tempView("شاهد")
    const first = cm.coordsForChar(0)!, last = cm.coordsForChar(cm.state.doc.length - 1)!
  ...
  it("doesn't include space taken up by widgets", () => {
    const cm = tempView("abc", [deco(Decoration.widget({widget: inline, side: 1}).range(1),
                                   Decoration.widget({widget: inline, side: -1}).range(2))])
    const a = cm.coordsForChar(0)!, b = cm.coordsForChar(1)!, c = cm.coordsForChar(2)!
    const ws = cm.contentDOM.querySelectorAll(".widget") as NodeListOf<HTMLElement>
    const w1 = ws[0].getBoundingClientRect(), w2 = ws[1].getBoundingClientRect()
    const ε = 0.01
  ...
  it("returns positions for wrap points", () => {
    const cm = tempView("aaaaaaaaa bbbbbbbbbbbbb", [EditorView.theme({"&": {maxWidth: "7em"}}), EditorView.lineWrapping])
    const a = cm.coordsForChar(0)!, b = cm.coordsForChar(10)!, wrap = cm.coordsForChar(9)!
    ist(a.top, b.top, "<")
    ist(wrap)
