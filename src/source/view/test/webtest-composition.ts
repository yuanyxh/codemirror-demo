...
function widgets(positions: number[], sides: number[]): ViewPlugin<any> {
  const xWidget = new class extends WidgetType {
    toDOM() { const s = document.createElement("var"); s.textContent = "×"; return s }
  }
...
describe("Composition", () => {
  it("supports composition on an empty line", () => {
    const cm = requireFocus(tempView("foo\n\nbar"))
  ...
  it("supports composition at start of line in a new node", () => {
    const cm = requireFocus(tempView("foo"))
    compose(cm, () => {
      const l0 = cm.contentDOM.firstChild!
  ...
  it("can deal with Android-style newline-after-composition", () => {
    const cm = requireFocus(tempView("abcdef"))
    compose(cm, () => up(cm.domAtPos(2).node as Text), [
      n => up(n, "x", 3),
      n => up(n, "y", 4)
    ], {end: n => {
      const line = n.parentNode!.appendChild(document.createElement("div"))
  ...
  it("handles replacement of existing words", () => {
    const cm = requireFocus(tempView("one two three"))
  ...
  it("doesn't get interrupted by changes in decorations", () => {
    const cm = requireFocus(tempView("foo ...", [wordHighlighter]))
  ...
  it("works inside highlighted text", () => {
    const cm = requireFocus(tempView("one two", [wordHighlighter]))
  ...
  it("doesn't overwrite widgets next to the composition", () => {
    const cm = requireFocus(tempView("", [widgets([0, 0], [-1, 1])]))
    compose(cm, () => {
      const l0 = cm.domAtPos(0).node
  ...
  it("works for composition in the middle of a mark", () => {
    const cm = requireFocus(tempView("one three", [wordHighlighter, deco([Decoration.mark({class: "a"}).range(0, 9)])]))
    compose(cm, () => up(cm.domAtPos(4).node as Text, "-"), [n => {
      const a = n.parentNode as HTMLElement
      ist(a.className, "a")
  ...
  it("works when composition rewraps the middle of a mark", () => {
    const cm = requireFocus(tempView("one three", [wordHighlighter, deco([Decoration.mark({class: "a"}).range(0, 9)])]))
    compose(cm, () => {
      const space = cm.domAtPos(4).node as Text, a = space.parentNode as HTMLElement
      const wrap1 = a.cloneNode(), wrap2 = a.cloneNode()
  ...
  it("cancels composition when a change fully overlaps with it", () => {
    const cm = requireFocus(tempView("one\ntwo\nthree"))
  ...
  it("doesn't cancel composition when a line break is inserted in front of it", () => {
    const cm = requireFocus(tempView("one two three", [wordHighlighter]))
    compose(cm, () => up(cm.domAtPos(9).node as Text, "x"), [
      n => up(n, "y"),
  ...
  it("works before a block widget", () => {
    const widget = new class extends WidgetType {
      toDOM() { const d = document.createElement("div"); d.textContent = "---"; return d }
    }
    const cm = requireFocus(tempView("abcd", [deco([Decoration.widget({widget, side: 1}).range(2)])]))
  ...
  it("can handle browsers inserting new wrapper nodes around the composition", () => {
    const cm = requireFocus(tempView("one two", [wordHighlighter]))
    compose(cm, () => {
      const span = cm.domAtPos(1).node.parentNode!
      const wrap = span.appendChild(document.createElement("font"))
      const text = wrap.appendChild(document.createTextNode(""))
      return up(text, "1")
    }, [n => up(n, "2")])
  ...
  it("can handle siblings being moved into a new wrapper", () => {
    const cm = requireFocus(tempView("one two", [wordHighlighter]))
    compose(cm, () => {
      const span = cm.domAtPos(1).node.parentNode as HTMLElement
      const wrap = span.parentNode!.insertBefore(document.createElement("font"), span)
  ...
  it("handles compositions rapidly following each other", () => {
    const cm = requireFocus(tempView("one\ntwo"))
    event(cm, "compositionstart")
    const one = cm.domAtPos(1).node as Text
    up(one, "!")
    cm.observer.flush()
    event(cm, "compositionend")
    one.nodeValue = "one!!"
    const L2 = cm.contentDOM.lastChild
    event(cm, "compositionstart")
    const two = cm.domAtPos(7).node as Text
  ...
  it("doesn't try to apply multi-cursor composition in a single node", () => {
    const cm = requireFocus(tempView("onetwo"))
  ...
  it("can handle IME merging spans", () => {
    ...
    const cm = requireFocus(tempView("(hello)", [field]))
    cm.dispatch({selection: {anchor: 1, head: 6}})
    compose(cm, () => up(cm.domAtPos(2).node as Text, "a"), [
      n => {
        const sA = cm.contentDOM.querySelector(".a")!, sB = cm.contentDOM.querySelector(".b")!
        sB.remove()
  ...
  it("can handle IME extending text nodes", () => {
    const cm = requireFocus(tempView("x .a.", [wordHighlighter]))
    compose(cm, () => {
      const dot = cm.contentDOM.firstChild!.lastChild! as Text
      dot.textContent = " .a.."
