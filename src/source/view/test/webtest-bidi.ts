...
function queryBrowserOrder(strings: readonly string[]) {
  ...
  for (const str of strings) {
    const wrap = scratch.appendChild(document.createElement("div"))
  ...
  const ltr: (readonly number[])[] = [], rtl: (readonly number[])[] = []
  for (let i = 0; i < 2; i++) {
    const dest = i ? rtl : ltr
...
function ourOrder(order: readonly BidiSpan[], dir: Direction) {
  const result = []
  for (const span of dir == Direction.LTR ? order : order.slice().reverse()) {
...
function tests(dir: Direction) {
  ...
  describe(Direction[dir] + " motion", () => {
    for (let i = 0; i < cases.length; i++) {
      for (let forward = true;; forward = false) {
        it(cases[i] + (forward ? " forward" : " backward"), () => {
          const order = __test.computeOrder(cases[i], dir, [])
          const line = Text.of([cases[i]]).line(1)
          const seen = new Set<number>()
          const span = order[forward ? 0 : order.length - 1]
          let pos = EditorSelection.cursor(span.side(!forward, dir), span.forward(forward, dir) ? 1 : -1)
          for (;;) {
            const id = pos.head * (pos.assoc < 0 ? -1 : 1)
            ist(!seen.has(id))
            seen.add(id)
            const next = __test.moveVisually(line, order, dir, pos, forward)
            if (!next) break
            pos = next
          }
    ...
    it("handles extending characters", () => {
      const str = "aé̠őx 😎🙉 👨‍🎤💪🏽👩‍👩‍👧‍👦 🇩🇪🇫🇷"
      const points = [0, 1, 4, 6, 7, 8, 10, 12, 13, 18, 22, 33, 34, 38, 42]
      const line = Text.of([str]).line(1)
      const order = __test.computeOrder(str, Direction.LTR, [])
    ...
    it("handles a misplaced non-joiner without going in a loop", () => {
      const doc = "ءAB\u200cء", line = Text.of([doc]).line(1)
      const order = __test.computeOrder(doc, Direction.RTL, [])
...
function testIsolates(doc: string, extensions: Extension, expected: string) {
  const cm = tempView(doc, [extensions])
  cm.measure()
