...
describe("EditorView drawing", () => {
  ...
  it("redraws lazily", () => {
    const cm = tempView("one\ntwo\nthree")
    const line0 = cm.contentDOM.firstChild!, line1 = line0.nextSibling!, line2 = line1.nextSibling!
    const text0 = line0.firstChild!, text2 = line2.firstChild!
  ...
  it("draws BR nodes on empty lines", () => {
    const cm = tempView("one\n\ntwo")
    const emptyLine = cm.domAtPos(4).node
  ...
  it("only draws visible content", () => {
    ...
    const gap = cm.contentDOM.lastChild
  ...
  it("scrolls the selection into view when asked", () => {
    ...
    const box = cm.scrollDOM.getBoundingClientRect(), pos = cm.coordsAtPos(250)
    ist(box.top, pos?.top, "<=")
  ...
  it("can scroll ranges into view", () => {
    ...
    const pos100 = cm.coordsAtPos(100)
  ...
  it("keeps a drawn area around selection ends", () => {
    ...
    cm.focus()
    const text = cm.contentDOM.textContent!
    ist(text.length, 500, "<")
  ...
  it("can handle replace-all like events", () => {
    ...
    const cm = tempView(content), changes = []
    for (let i = Math.floor(content.length / 100); i >= 0; i--) {
      const from = Math.floor(Math.random() * (cm.state.doc.length - 10)), to = from + Math.floor(Math.random() * 10)
  ...
  it("correctly handles very complicated transactions", () => {
    let doc = "foo\nbar\nbaz", chars = "abcdef  \n"
    const cm = tempView(doc)
    for (let i = 0; i < 10; i++) {
      let changes = [], pos = Math.min(20, doc.length)
      for (let j = 0; j < 1; j++) {
        const choice = Math.random(), r = Math.random()
        if (choice < 0.15) {
          pos = Math.min(doc.length, Math.max(0, pos + 5 - Math.floor(r * 10)))
        } else if (choice < 0.5) {
          const from = Math.max(0, pos - Math.floor(r * 2)), to = Math.min(doc.length, pos + Math.floor(r * 4))
  ...
  it("notices it is added to the DOM even if initially detached", () => {
    ...
    const cm = new EditorView({doc: "a\nb\nc\nd"})
    const defaultHeight = cm.contentHeight
  ...
  it("doesn't overcompensate scroll position for poorly estimated height", () => {
    ...
    cm.measure()
    const prev = cm.scrollDOM.scrollTop
  ...
  it("hides parts of long lines that are vertically out of view", () => {
    const cm = tempView("<" + "long line ".repeat(10e3) + ">", [scroll(100), EditorView.lineWrapping])
    cm.measure()
  ...
  it("properly attaches styles in shadow roots", () => {
    ...
    const wrap = ws.appendChild(document.createElement("div"))
    if (!wrap.attachShadow) return
    const shadow = wrap.attachShadow({mode: "open"})
    const editor = new EditorView({root: shadow})
  ...
  it("updates height info when a widget changes size", async () => {
    ...
    cm.measure()
    await later(75)
    const line2 = cm.viewportLineBlocks[1], dom2 = cm.contentDOM.querySelectorAll(".cm-line")[1]
