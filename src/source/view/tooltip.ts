  ...
  class {
    ...
    writeMeasure(measured: Measured) {
      ...
      for (let i = 0; i < this.manager.tooltips.length; i++) {
        ...
        if (arrow) {
          const arrowLeft =
            pos.left + (ltr ? offset.x : -offset.x) - (left + Arrow.Offset - Arrow.Size);
...
class HoverPlugin {
  ...
  get tooltip() {
    ...
    const index = plugin
      ? plugin.manager.tooltips.findIndex((t) => t.create == HoverTooltipHost.create)
      : -1;
...
export function hoverTooltip(
  ...
  return {
    ...
    extension: [
      ...
      ViewPlugin.define(
        (view) =>
          new HoverPlugin(view, source, hoverState, setHover, options.hoverTime || Hover.Time)
      ),
