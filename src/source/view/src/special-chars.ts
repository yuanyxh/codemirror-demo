...
const specialCharConfig = Facet.define<SpecialCharConfig, Required<SpecialCharConfig> & {replaceTabs?: boolean}>({
  combine(configs) {
    const config: Required<SpecialCharConfig> & {replaceTabs?: boolean} = combineConfig(configs, {
      render: null,
      ...
      return new MatchDecorator({
        ...
        decoration: (m, view, pos) => {
          ...
          if (code == 9) {
            ...
            const size = view.state.tabSize, col = countColumn(line.text, size, pos - line.from)
...
class SpecialCharWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    const desc = view.state.phrase("Control character") + " " + (Names[this.code] || "0x" + this.code.toString(16))
