...
export class HeightOracle {
  ...
  refresh(whiteSpace: string, lineHeight: number, charWidth: number, textHeight: number,
    ...
    const changed = Math.round(lineHeight) != Math.round(this.lineHeight) || this.lineWrapping != lineWrapping
...
export class BlockInfo {
  ...
  join(other: BlockInfo) {
    const content = (Array.isArray(this._content) ? this._content : [this])
...
class HeightMapGap extends HeightMap {
  ...
  private heightMetrics(oracle: HeightOracle, offset: number): {
    ...
    const firstLine = oracle.doc.lineAt(offset).number, lastLine = oracle.doc.lineAt(offset + this.length).number
  ...
  blockAt(height: number, oracle: HeightOracle, top: number, offset: number) {
    ...
    if (oracle.lineWrapping) {
      const guess = offset + (height < oracle.lineHeight ? 0
        : Math.round(Math.max(0, Math.min(1, (height - top) / this.height)) * this.length))
      const line = oracle.doc.lineAt(guess), lineHeight = perLine + line.length * perChar
      const lineTop = Math.max(top, height - lineHeight / 2)
      return new BlockInfo(line.from, line.length, lineTop, lineHeight, 0)
    } else {
      const line = Math.max(0, Math.min(lastLine - firstLine, Math.floor((height - top) / perLine)))
  ...
  lineAt(value: number, type: QueryType, oracle: HeightOracle, top: number, offset: number) {
    ...
    const line = oracle.doc.lineAt(value), lineHeight = perLine + line.length * perChar
...
class HeightMapBranch extends HeightMap {
  ...
  lineAt(value: number, type: QueryType, oracle: HeightOracle, top: number, offset: number) {
    const rightTop = top + this.left.height, rightOffset = offset + this.left.length + this.break
    const left = type == QueryType.ByHeight ? value < rightTop : value < rightOffset
    const base = left ? this.left.lineAt(value, type, oracle, top, offset)
  ...
  decomposeRight(from: number, result: (HeightMap | null)[]) {
    const left = this.left.length, right = left + this.break
...
class NodeBuilder implements SpanIterator<Decoration> {
  ...
  span(_from: number, to: number) {
    if (this.lineStart > -1) {
      const end = Math.min(to, this.lineEnd), last = this.nodes[this.nodes.length - 1]
