...
export class DOMObserver {
  ...
  onSelectionChange(event: Event) {
    ...
    const {view} = this, sel = this.selectionRange
  ...
  readSelectionRange() {
    ...
    const range = browser.safari && (view.root as any).nodeType == 11 &&
  ...
  listenForScroll() {
    ...
    if (changed) {
      ...
      for (const dom of this.scrollTargets = changed) dom.addEventListener("scroll", this.onScroll)
    }
  }
...
function safariSelectionRangeHack(view: EditorView, selection: Selection) {
  if ((selection as any).getComposedRanges) {
    const range = (selection as any).getComposedRanges(view.root)[0] as StaticRange
...
class EditContextManager {
  ...
  constructor(view: EditorView) {
    ...
    this.handlers.textupdate = e => {
      ...
      const change = {from: this.toEditorPos(e.updateRangeStart),
    ...
    this.handlers.textformatupdate = e => {
      ...
      for (const format of e.getTextFormats()) {
        const lineStyle = format.underlineStyle, thickness = format.underlineThickness
  ...
  setSelection(state: EditorState) {
    ...
    const start = this.toContextPos(Math.max(this.from, Math.min(this.to, main.anchor)))
  ...
  destroy() {
    for (const event in this.handlers) this.editContext.removeEventListener(event as any, this.handlers[event])
  }
}
