  ...
  constructor(view: EditorView, start: number, end: number, readonly typeOver: boolean) {
    ...
    if (view.state.readOnly && start > -1) {
      ...
      if ((browser.ios || browser.chrome) && view.state.selection.main.empty && head != anchor &&
        ...
        const from = Math.min(head, anchor), to = Math.max(head, anchor)
        const offFrom = vp.from - from, offTo = vp.to - to
...
export function applyDOMChange(view: EditorView, domChange: DOMChange): boolean {
  ...
  if (domChange.bounds) {
    ...
    const diff = findDiff(view.state.doc.sliceString(from, to, LineBreakPlaceholder), domChange.text,
...
function applyDefaultInsert(view: EditorView, change: {from: number, to: number, insert: Text},
  ...
  if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3 &&
    ...
    if (startState.selection.ranges.length > 1 && view.inputState.composing >= 0 &&
      ...
      const offset = sel.to - change.to, size = sel.to - sel.from
      tr = startState.changeByRange(range => {
        if (range.from == sel.from && range.to == sel.to)
          return {changes, range: mainSel || range.map(changes)}
        const to = range.to - offset, from = to - replaced.length
        if (range.to - range.from != size || view.state.sliceDoc(from, to) != replaced ||
          ...
          return {range}
        const rangeChanges = startState.changes({from, to, insert: change!.insert}), selOff = range.to - sel.to
        return {
...
function selectionFromPoints(points: DOMPoint[], base: number): EditorSelection | null {
  ...
  const anchor = points[0].pos, head = points.length == 2 ? points[1].pos : anchor
