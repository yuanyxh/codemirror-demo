...
function rectanglesForRange(view: EditorView, className: string, range: SelectionRange): RectangleMarker[] {
  ...
  const content = view.contentDOM, contentRect = content.getBoundingClientRect(), base = getBase(view)
  const lineElt = content.querySelector(".cm-line"), lineStyle = lineElt && window.getComputedStyle(lineElt)
  const leftSide = contentRect.left +
    (lineStyle ? parseInt(lineStyle.paddingLeft) + Math.min(0, parseInt(lineStyle.textIndent)) : 0)
  const rightSide = contentRect.right - (lineStyle ? parseInt(lineStyle.paddingRight) : 0)

  const startBlock = blockAt(view, from), endBlock = blockAt(view, to)
  ...
  function drawForLine(from: null | number, to: null | number, line: {from: number, to: number}) {
    ...
    function addSpan(from: number, fromOpen: boolean, to: number, toOpen: boolean, dir: Direction) {
      ...
      const fromCoords = view.coordsAtPos(from, (from == line.to ? -2 : 2) as any)
    ...
    const start = from ?? line.from, end = to ?? line.to
    // Split the range by visible range and document line
    for (const r of view.visibleRanges) if (r.to > start && r.from < end) {
      for (let pos = Math.max(r.from, start), endPos = Math.min(r.to, end);;) {
        const docLine = view.state.doc.lineAt(pos)
        for (const span of view.bidiSpans(docLine)) {
          const spanFrom = span.from + docLine.from, spanTo = span.to + docLine.from
