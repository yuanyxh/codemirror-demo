...
export class EditorView {
  ...
  update(transactions: readonly Transaction[]) {
    ...
    try {
      ...
      for (const tr of transactions) {
        ...
        for (const e of tr.effects) if (e.is(scrollIntoView)) scrollTarget = e.value.clip(this.state);
      }
  ...
  private updatePlugins(update: ViewUpdate) {
    ...
    if (prevSpecs != specs) {
      ...
      for (const plugin of this.plugins) if (plugin.mustUpdate != update) plugin.destroy(this);
