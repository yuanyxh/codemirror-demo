...
export class ViewState {
  ...
  measure(view: EditorView) {
    ...
    // Pixel viewport
    const pixelViewport = (this.printing ? fullPixelRange : visiblePixelRange)(dom, this.paddingTop);
    ...
    const checkLine = (line: BlockInfo) => {
      ...
      if (wrapping) {
        ...
        if (target != null) {
          ...
          const spaceFrac = ((this.visibleBottom - this.visibleTop) / 2 + marginHeight) / line.height;
