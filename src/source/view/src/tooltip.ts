...
class TooltipViewManager {
  ...
  update(update: ViewUpdate, above?: boolean[]) {
    ...
    const tooltipViews: TooltipView[] = [], newAbove: boolean[] | null = above ? [] : null
    ...
    for (const t of this.tooltipViews) if (tooltipViews.indexOf(t) < 0) {
...
const tooltipPlugin = ViewPlugin.fromClass(class {
  ...
  update(update: ViewUpdate) {
    ...
    if (newConfig.position != this.position && !this.madeAbsolute) {
      ...
      for (const t of this.manager.tooltipViews) t.dom.style.position = this.position
      shouldMeasure = true
    }
    if (newConfig.parent != this.parent) {
      ...
      this.createContainer()
      for (const t of this.manager.tooltipViews) this.container.appendChild(t.dom)
      shouldMeasure = true
  ...
  writeMeasure(measured: Measured) {
    if (measured.makeAbsolute) {
      ...
      for (const t of this.manager.tooltipViews) t.dom.style.position = "absolute"
    }

    const {editor, space, scaleX, scaleY} = measured
    const others = []
    for (let i = 0; i < this.manager.tooltips.length; i++) {
      const tooltip = this.manager.tooltips[i], tView = this.manager.tooltipViews[i], {dom} = tView
      const pos = measured.pos[i], size = measured.size[i]
      ...
      const arrow: HTMLElement | null = tooltip.arrow ? tView.dom.querySelector(".cm-tooltip-arrow") : null
      const arrowHeight = arrow ? Arrow.Size : 0
      let width = size.right - size.left, height = knownHeight.get(tView) ?? size.bottom - size.top
      const offset = tView.offset || noOffset, ltr = this.view.textDirection == Direction.LTR
      const left = size.width > space.right - space.left
      ...
      const spaceVert = (above ? pos.top - space.top : space.bottom - pos.bottom) - arrowHeight
      ...
      if (tView.overlap !== true) for (const r of others)
      ...
      if (arrow) {
        const arrowLeft = pos.left + (ltr ? offset.x : -offset.x) - (left + Arrow.Offset - Arrow.Size)
  ...
  maybeMeasure() {
    if (this.manager.tooltips.length) {
      ...
      if (this.inView != this.view.inView) {
        ...
        if (!this.inView) for (const tv of this.manager.tooltipViews) tv.dom.style.top = Outside
      }
    }
  }
}, {
...
class HoverPlugin {
  ...
  startHover() {
    ...
    if (desc instanceof WidgetView) {
      ...
      const bidi = view.bidiSpans(view.state.doc.lineAt(pos)).find(s => s.from <= pos! && s.to >= pos!)
  ...
  get tooltip() {
    ...
    const index = plugin ? plugin.manager.tooltips.findIndex(t => t.create == HoverTooltipHost.create) : -1
  ...
  mousemove(event: MouseEvent) {
    ...
    if (active.length && tooltip && !isInTooltip(tooltip.dom, event) || this.pending) {
      const {pos} = active[0] || this.pending!, end = active[0]?.end ?? pos
  ...
  mouseleave(event: MouseEvent) {
    ...
    if (active.length) {
      const {tooltip} = this
      const inTooltip = tooltip && tooltip.dom.contains(event.relatedTarget as HTMLElement)
      if (!inTooltip)
        this.view.dispatch({effects: this.setHover.of([])})
      else
...
function isOverRange(view: EditorView, from: number, to: number, x: number, y: number, margin: number) {
  ...
  const docBottom = view.documentTop + view.documentPadding.top + view.contentHeight
