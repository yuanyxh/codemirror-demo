...
export function getIsolatedRanges(view: EditorView, line: Line): readonly Isolate[] {
  ...
  RangeSet.spans(sets, line.from, line.to, {
    ...
    span(fromDoc, toDoc, active, open) {
      const from = fromDoc - line.from, to = toDoc - line.from
      let level = result
...
export class ChangedRange {
  ...
  static extendWithRanges(diff: readonly ChangedRange[], ranges: number[]): readonly ChangedRange[] {
    ...
    for (let dI = 0, rI = 0, posA = 0, posB = 0;; dI++) {
      const next = dI == diff.length ? null : diff[dI], off = posA - posB
      const end = next ? next.fromB : 1e9
      while (rI < ranges.length && ranges[rI] < end) {
        const from = ranges[rI], to = ranges[rI + 1]
        const fromB = Math.max(posB, from), toB = Math.min(end, to)
