...
export class MatchDecorator {
  ...
  createDeco(view: EditorView) {
    const build = new RangeSetBuilder<Decoration>(), add = build.add.bind(build)
  ...
  private updateRange(view: EditorView, deco: DecorationSet, updateFrom: number, updateTo: number) {
    for (const r of view.visibleRanges) {
      ...
      if (to > from) {
        const fromLine = view.state.doc.lineAt(from), toLine = fromLine.to < to ? view.state.doc.lineAt(to) : fromLine
        ...
        const add = (from: number, to: number, deco: Decoration) => ranges.push(deco.range(from, to))
        if (fromLine == toLine) {
