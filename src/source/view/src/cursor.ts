...
function domPosAtCoords(parent: HTMLElement, x: number, y: number): {node: Node, offset: number} {
  ...
  for (let child: Node | null = parent.firstChild; child; child = child.nextSibling) {
    ...
    for (let i = 0; i < rects.length; i++) {
      ...
      const dx = getdx(x, rect), dy = getdy(y, rect)
      if (dx == 0 && dy == 0)
      ...
      if (!closest || closestY > dy || closestY == dy && closestX > dx) {
        ...
        const side = dy ? (y < rect.top ? -1 : 1) : dx ? (x < rect.left ? -1 : 1) : 0
  ...
  const offset = Array.prototype.indexOf.call(parent.childNodes, closest) +
...
export function posAtCoords(view: EditorView, coords: {x: number, y: number}, precise: boolean, bias: -1 | 1 = -1): number | null {
  const content = view.contentDOM.getBoundingClientRect(), docTop = content.top + view.viewState.paddingTop
  ...
  const root = (view.root as any).elementFromPoint ? view.root as Document : doc
...
function posAtCoordsImprecise(view: EditorView, contentRect: Rect, block: BlockInfo, x: number, y: number) {
  ...
  if (view.lineWrapping && block.height > view.defaultLineHeight * 1.5) {
    ...
    const line = Math.floor((y - block.top - (view.defaultLineHeight - textHeight) * 0.5) / textHeight)
...
function isSuspiciousChromeCaretResult(node: Node, offset: number, x: number) {
  ...
  const rect = node.nodeType == 1 ? (node as HTMLElement).getBoundingClientRect()
...
export function moveToLineBoundary(view: EditorView, start: SelectionRange, forward: boolean, includeWrap: boolean) {
  ...
  const coords = !includeWrap || line.type != BlockType.Text || !(view.lineWrapping || line.widgetLineBreaks) ? null
    : view.coordsAtPos(start.assoc < 0 && start.head > line.from ? start.head - 1 : start.head)
  if (coords) {
    const editorRect = view.dom.getBoundingClientRect()
    const direction = view.textDirectionAt(line.from)
    const pos = view.posAtCoords({x: forward == (direction == Direction.LTR) ? editorRect.right - 1 : editorRect.left + 1,
...
export function moveVertically(view: EditorView, start: SelectionRange, forward: boolean, distance?: number) {
  const startPos = start.head, dir: -1 | 1 = forward ? 1 : -1
  if (startPos == (forward ? view.state.doc.length : 0)) return EditorSelection.cursor(startPos, start.assoc)
  let goal = start.goalColumn, startY
  const rect = view.contentDOM.getBoundingClientRect()
  const startCoords = view.coordsAtPos(startPos, start.assoc || -1), docTop = view.documentTop
  if (startCoords) {
...
export function skipAtoms(view: EditorView, oldPos: SelectionRange, pos: SelectionRange) {
  const newPos = skipAtomicRanges(view.state.facet(atomicRanges).map(f => f(view)), pos.from, oldPos.head > pos.from ? -1 : 1)
