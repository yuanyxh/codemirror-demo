...
export class DocView extends ContentView {
  ...
  update(update: ViewUpdate) {
    ...
    const composition = readCompositionAt > -1 ? findCompositionRange(this.view, update.changes, readCompositionAt) : null
    ...
    const prevDeco = this.decorations, deco = this.updateDeco()
  ...
  private updateInner(changes: readonly ChangedRange[], oldLength: number, composition: Composition | null) {
    ...
    observer.ignore(() => {
      ...
      const track = browser.chrome || browser.ios ? {node: observer.selectionRange.focusNode!, written: false} : undefined
    ...
    const gaps = []
    if (this.view.viewport.from || this.view.viewport.to < this.view.state.doc.length) for (const child of this.children)
  ...
  private updateChildren(changes: readonly ChangedRange[], oldLength: number, composition: Composition | null) {
    ...
    for (let i = ranges.length - 1;; i--) {
      ...
      if (composition && composition.range.fromB < toB && composition.range.toB > fromB) {
        const before = ContentBuilder.build(this.view.state.doc, fromB, composition.range.fromB, this.decorations,
                                          this.dynamicDecorationMap)
        const after = ContentBuilder.build(this.view.state.doc, composition.range.toB, toB, this.decorations,
                                         this.dynamicDecorationMap)
  ...
  private updateEditContextFormatting(update: ViewUpdate) {
    ...
    for (const tr of update.transactions) for (const effect of tr.effects) if (effect.is(setEditContextFormatting)) {
  ...
  updateSelection(mustRead = false, fromPointer = false) {
    ...
    const activeElt = this.view.root.activeElement, focused = activeElt == this.dom
    const selectionNotFocus = !focused &&
        ...
        if (!rawSel) {
          ...
          if (browser.gecko) {
            ...
            if (nextTo && nextTo != (NextTo.Before | NextTo.After)) {
              const text = (nextTo == NextTo.Before ? textNodeBefore : textNodeAfter)(anchor.node, anchor.offset)
  ...
  enforceCursorAssoc() {
    ...
    const {view} = this, cursor = view.state.selection.main
    ...
    const before = this.coordsAt(cursor.head, -1), after = this.coordsAt(cursor.head, 1)
  ...
  coordsAt(pos: number, side: number): Rect | null {
    ...
    for (let off = this.length, i = this.children.length - 1; i >= 0; i--) {
      const child = this.children[i], end = off - child.breakAfter, start = end - child.length
      if (end < pos) break
  ...
  measureVisibleLineHeights(viewport: {from: number, to: number}) {
    ...
    for (let pos = 0, i = 0; i < this.children.length; i++) {
      const child = this.children[i], end = pos + child.length
      if (end > to) break
      if (pos >= from) {
  ...
  computeBlockGapDeco(): DecorationSet {
    const deco = [], vs = this.view.viewState
    for (let pos = 0, i = 0;; i++) {
