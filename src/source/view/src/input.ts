...
export class InputState {
  ...
  ensureHandlers(plugins: readonly PluginInstance[]) {
    const handlers = computeHandlers(plugins), prev = this.handlers, dom = this.view.contentDOM
    for (const type in handlers) if (type != "scroll") {
    ...
    for (const type in prev) if (type != "scroll" && !handlers[type])
      dom.removeEventListener(type, this.handleEvent)
...
function computeHandlers(plugins: readonly PluginInstance[]) {
  ...
  for (const plugin of plugins) {
    ...
    if (spec && spec.domEventHandlers) for (const type in spec.domEventHandlers) {
      const f = spec.domEventHandlers[type]
      if (f) record(type).handlers.push(bindHandler(plugin.value!, f))
    }
    if (spec && spec.domEventObservers) for (const type in spec.domEventObservers) {
...
class MouseSelection {
  ...
  select(event: MouseEvent) {
    const {view} = this, selection = this.skipAtoms(this.style.get(event, this.extend, this.multiple))
...
function doPaste(view: EditorView, input: string) {
  ...
  const linewise = lastLinewiseCopy != null && state.selection.ranges.every(r => r.empty) && lastLinewiseCopy == text.toString()
  if (linewise) {
    let lastLine = -1
...
function rangeForClick(view: EditorView, pos: number, bias: -1 | 1, type: number): SelectionRange {
  if (type == 1) { // Single click
    ...
    const visual = LineView.find(view.docView, pos), line = view.state.doc.lineAt(visual ? visual.posAtEnd : pos)
...
const inside = (x: number, y: number, rect: Rect) => y >= rect.top && y <= rect.bottom && x >= rect.left && x <= rect.right
...
function getClickType(event: MouseEvent) {
  ...
  const last = lastMouseDown, lastTime = lastMouseDownTime
  lastMouseDown = event
...
function basicMouseSelection(view: EditorView, event: MouseEvent) {
  const start = queryPos(view, event), type = getClickType(event)
...
handlers.dragstart = (view, event: DragEvent) => {
  ...
  if ((event.target as HTMLElement).draggable) {
    ...
    if (cView && cView.isWidget) {
      const from = cView.posAtStart, to = from + cView.length
...
function dropText(view: EditorView, event: DragEvent, text: string, direct: boolean) {
  ...
  const del = direct && draggedContent && dragMovesSelection(view, event)
...
handlers.beforeinput = (view, event: InputEvent) => {
  ...
  if (event.inputType == "insertReplacementText" && view.observer.editContext) {
    const text = event.dataTransfer?.getData("text/plain"), ranges = event.getTargetRanges()
    if (text && ranges.length) {
      const r = ranges[0]
      const from = view.posAtDOM(r.startContainer, r.startOffset), to = view.posAtDOM(r.endContainer, r.endOffset)
