  ...
  private getIndent(cx: IndentContext, pos: number) {
    ...
    while (statePos < pos) {
      ...
      if (line.length) {
        ...
        const stream = new StringStream(line.text, cx.state.tabSize, cx.unit, indentation < 0 ? undefined : indentation)
...
class Parse<State> implements PartialParse {
  ...
  parseLine(context: ParseContext | null) {
    ...
    const stream = new StringStream(line, context ? context.state.tabSize : 4, context ? getIndentUnit(context.state) : 2)
    if (stream.eol()) {
...
function createTokenType(extra: {[name: string]: Tag | readonly Tag[]}, tagStr: string) {
  ...
  for (const name of tagStr.split(" ")) {
    ...
    for (const part of name.split(".")) {
      const value = (extra[part] || (highlightTags as any)[part]) as Tag | readonly Tag[] | ((t: Tag) => Tag) | undefined
      if (!value) {
  ...
  const name = tagStr.replace(/ /g, "_"), key = name + " " + tags.map(t => (t as any).id)
...
function docID(data: Facet<{[name: string]: any}>) {
  const type = NodeType.define({id: typeArray.length, name: "Document", props: [languageDataProp.add(() => data)], top: true})
  typeArray.push(type)
  return type
}
