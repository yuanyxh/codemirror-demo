...
describe("history", () => {
  ...
  it("supports a custom join predicate", () => {
    let state = mkState({
      joinToEvent: (tr: Transaction, adj: boolean) => {
        if (!adj) return false;
        let space = false;
        if (adj)
          tr.changes.iterChanges((fA, tA, fB, tB, text) => {
            if (text.length && text.sliceString(0, 1) == " ") space = true;
          });
        return !space;
      },
    });
    for (const ch of "ab cd") state = type(state, ch);
  ...
  it("accurately maps changes through each other", () => {
    ...
    state = state.update({
      changes: [
        ...
        { from: 1, to: 2, insert: "cd" },
        { from: 2, to: 3, insert: "ef" },
      ],
    }).state;
  ...
  it("restores selection on redo", () => {
    let state = mkState({}, "a\nb\nc\n");
    state = state.update({
      selection: EditorSelection.create([1, 3, 5].map((n) => EditorSelection.cursor(n))),
    }).state;
  ...
  describe("effects", () => {
    it("includes inverted effects in the history", () => {
      const set = StateEffect.define<number>();
      const field = StateField.define({
        create: () => 0,
        update(val, tr) {
          for (const effect of tr.effects) if (effect.is(set)) val = effect.value;
          return val;
        },
      });
      const invert = invertedEffects.of((tr) => {
        for (const e of tr.effects) if (e.is(set)) return [set.of(tr.startState.field(field))];
        return [];
      });
    ...
    function mapComment(comment: Comment, mapping: ChangeDesc) {
      const from = mapping.mapPos(comment.from, 1),
        to = mapping.mapPos(comment.to, -1);
      return from >= to ? undefined : new Comment(from, to, comment.text);
    }
    const addComment: StateEffectType<Comment> = StateEffect.define<Comment>({ map: mapComment });
    const rmComment: StateEffectType<Comment> = StateEffect.define<Comment>({ map: mapComment });
    const comments = StateField.define<Comment[]>({
      create: () => [],
      update(value, tr) {
    ...
    const invertComments = invertedEffects.of((tr) => {
      const effects = [];
      for (const effect of tr.effects) {
        if (effect.is(addComment) || effect.is(rmComment)) {
          const src = mapComment(effect.value, tr.changes.invertedDesc);
          if (src) effects.push((effect.is(addComment) ? rmComment : addComment).of(src));
        }
      }
      for (const comment of tr.startState.field(comments)) {
        if (!mapComment(comment, tr.changes)) effects.push(addComment.of(comment));
      }
      return effects;
    });

    function commentStr(state: EditorState) {
      return state
        .field(comments)
        .map((c) => c.text + "@" + c.from)
        .join(",");
    ...
    it("can map effects", () => {
      let state = EditorState.create({
        extensions: [history(), comments, invertComments],
        doc: "one two foo",
      });
      state = state.update({
        effects: addComment.of(new Comment(0, 3, "c1")),
        annotations: isolateHistory.of("full"),
      }).state;
      ...
      state = state.update({
        ...
        annotations: isolateHistory.of("full"),
        effects: addComment.of(new Comment(6, 9, "c2")),
      }).state;
      ...
      ist(commentStr(state), "c1@3,c2@9");
      ist(state.doc.toString(), "---one---two foo");
      state = command(state, undo).update({
        changes: { from: 10, to: 11, insert: "---" },
        annotations: Transaction.addToHistory.of(false),
      }).state;
      state = state.update({
        effects: addComment.of(new Comment(13, 16, "c3")),
        annotations: isolateHistory.of("full"),
      }).state;
    ...
    it("can restore comments lost through deletion", () => {
      ...
      state = state.update({
        effects: addComment.of(new Comment(3, 5, "c1")),
        annotations: isolateHistory.of("full"),
      }).state;
  ...
  describe("JSON", () => {
    it("survives serialization", () => {
      ...
      state = command(state, undo);
      const jsonConf = { history: historyField };
      const json = JSON.stringify(state.toJSON(jsonConf));
