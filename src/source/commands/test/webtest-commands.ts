import { EditorView, Command } from "@/view/index";
import { Extension, EditorState } from "@/state/index";
...
function testCmd(before: string, after: string, command: Command, extensions: Extension = []) {
  const state = mkState(before, extensions);
...
describe("commands", () => {
  describe("cursorSubwordForward", () => {
    ...
    it("stops at the end of word", () => testCmd("o|kay.", "okay|.", cursorSubwordForward));
  ...
  describe("cursorSubwordBackward", () => {
    ...
    it("stops at the end of word", () => testCmd(".o|kay", ".|okay", cursorSubwordBackward));
