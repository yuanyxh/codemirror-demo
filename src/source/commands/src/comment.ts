...
function changeBlockComment(
  ...
  const tokens = ranges.map(r => getConfig(state, r.from).block) as {open: string, close: string}[]
  if (!tokens.every(c => c)) return null
  const comments = ranges.map((r, i) => findBlockComment(state, tokens[i], r.from, r.to))
  if (option != CommentOption.Uncomment && !comments.every(c => c)) {
    ...
    for (let i = 0, comment; i < comments.length; i++) if (comment = comments[i]) {
      const token = tokens[i], {open, close} = comment
      changes.push(
...
function changeLineComment(
  ...
  const lines: {line: Line, token: string, comment: number, empty: boolean, indent: number, single: boolean}[] = []
  let prevLine = -1
  for (const {from, to} of ranges) {
    ...
    for (let pos = from; pos <= to;) {
      ...
      if (line.from > prevLine && (from == to || to > line.from)) {
        ...
        const comment = line.text.slice(indent, indent + token.length) == token ? indent : -1
  ...
  if (option != CommentOption.Uncomment && lines.some(l => l.comment < 0 && (!l.empty || l.single))) {
    const changes = []
    for (const {line, token, indent, empty, single} of lines) if (single || !empty)
      changes.push({from: line.from + indent, insert: token + " "})
    const changeSet = state.changes(changes)
    return {changes: changeSet, selection: state.selection.map(changeSet, 1)}
  } else if (option != CommentOption.Comment && lines.some(l => l.comment >= 0)) {
    const changes = []
    for (const {line, comment, token} of lines) if (comment >= 0) {
