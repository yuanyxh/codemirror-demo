...
function moveBySubword(view: EditorView, range: SelectionRange, forward: boolean) {
  ...
  if (segmenter && cat == CharCategory.Word as any && end.from == range.from + steps * (forward ? 1 : -1)) {
    const from = Math.min(range.head, end.head), to = Math.max(range.head, end.head)
    const skipped = view.state.sliceDoc(from, to)
    if (skipped.length > 1 && /[\u4E00-\uffff]/.test(skipped)) {
      const segments = Array.from(segmenter.segment(skipped)) as {index: number}[]
...
function toMatchingBracket(state: EditorState, dispatch: (tr: Transaction) => void, extend: boolean) {
  let found = false, selection = updateSel(state.selection, range => {
    const matching = matchBrackets(state, range.head, -1)
...
export const selectLine: StateCommand = ({state, dispatch}) => {
  const ranges = selectedLineBlocks(state).map(({from, to}) => EditorSelection.range(from, Math.min(to + 1, state.doc.length)))
...
export const indentSelection: StateCommand = ({state, dispatch}) => {
  ...
  const context = new IndentContext(state, {overrideIndentation: start => {
