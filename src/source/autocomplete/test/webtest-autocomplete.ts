...
class Runner {
  ...
  runTest(name: string, spec: TestSpec, f: (view: EditorView, sync: Sync) => Promise<void>) {
    ...
    const selection = spec.selection == null ? EditorSelection.single((spec.doc || "").length)
    ...
    const sync = (get: (state: EditorState) => any, value: any) => new Promise<void>((resolve, reject) => {
  ...
  async finish(filter?: string) {
    ...
    for (let from = 0; from < tests.length; from += Chunk) {
      const active = tests.slice(from, Math.min(tests.length, from + Chunk)).map(t => this.runTest(t.name, t.spec, t.f))
      const cleanup = () => {
