...
import { EditorState, EditorSelection } from "@/state/index";
import {
  CompletionSource,
  autocompletion,
  CompletionContext,
...
class Runner {
  ...
  runTest(name: string, spec: TestSpec, f: (view: EditorView, sync: Sync) => Promise<void>) {
    let syncing: { get: (state: EditorState) => any; value: any; resolve: () => void } | null =
      null;
    const selection =
      spec.selection == null
        ? EditorSelection.single((spec.doc || "").length)
        : typeof spec.selection == "number"
        ? EditorSelection.single(spec.selection)
        : spec.selection;
    ...
    const sync = (get: (state: EditorState) => any, value: any) =>
      new Promise<void>((resolve, reject) => {
        if (syncing) throw new Error("Overlapping syncs");
        if (get(view.state) === value) return resolve();
        const mine = (syncing = { get, value, resolve });
        setTimeout(() => {
          if (syncing == mine)
            reject(new Error(`${name}: Failed to sync: ${get(view.state)} !== ${value}\n`));
        }, Timeout);
      });
  ...
  async finish(filter?: string) {
    ...
    for (let from = 0; from < tests.length; from += Chunk) {
      const active = tests
        .slice(from, Math.min(tests.length, from + Chunk))
        .map((t) => this.runTest(t.name, t.spec, t.f));
      const cleanup = () => {
        for (const { view } of active) view.destroy();
      };
      await Promise.all(active.map((t) => t.promise)).then(cleanup, (err) => {
        cleanup();
        throw err;
      });
    }
...
function from(list: string): CompletionSource {
  return (cx) => {
    ...
    return {
      ...
      options: list.split(" ").map((w) => ({ label: w })),
      validFor: /^\w*/,
    };
  };
}
...
function tagged(validFor: boolean): CompletionSource {
  return (cx) => {
    ...
    return {
      from: word ? word.from : cx.pos,
      options: [{ label: "tag" + cx.pos }],
      validFor: validFor ? /^\w*/ : undefined,
    };
  };
}
...
function slow(c: CompletionSource, delay: number): CompletionSource {
  return (cx: CompletionContext) =>
    new Promise((resolve) => setTimeout(() => resolve(c(cx)), delay));
}
...
function options(s: EditorState) {
  return currentCompletions(s)
    .map((c) => (/ /.test(c.label) ? JSON.stringify(c.label) : c.label))
    .join(" ");
}
...
describe("autocomplete", () => {
  ...
  it("works", function () {
    ...
    run.options(
      ...
      "one",
      [from(words), from("onet bonae")],
      "one onet onetwothree OneTwoThree bonae"
    );

    run.options(
      "only shows prefix matches for single-letter queries",
      "t",
      [from(words)],
      "three two"
    );
    ...
    run.options(
      "prefers case-matched completions",
      "eTw",
      [from(words)],
    ...
    run.options(
      "handles all-uppercase words",
      "sel",
      [from("SCOPE_CATALOG SELECT SELECTIVE")],
      "SELECT SELECTIVE SCOPE_CATALOG"
    );

    run.options(
      "penalizes by-word matches with gaps",
      "abc",
      [from("xabc aVeryBigCar")],
      "xabc aVeryBigCar"
    );

    run.options(
      "prefers shorter options",
      "hair",
    ...
    run.test(
      "will eagerly populate the result list when a source is slow",
      {
        doc: "on",
    ...
    run.test(
      "further narrows completions on input",
      { sources: [once(from("one okay ono"))] },
      async (view, sync) => {
        type(view, "o");
    ...
    run.test(
      "doesn't abort on backspace",
      { sources: [once(from("one okay")), once(from("ohai"))] },
      async (view, sync) => {
        type(view, "on");
        await sync(options, "one");
        del(view);
    ...
    run.test(
      "can backspace out entire word when explicit",
    ...
    run.test(
      "stops explicit completion on non-spanning input",
      { sources: [from("one two")] },
      async (view, sync) => {
        startCompletion(view);
        await sync(options, "one two");
    ...
    run.test(
      "stops explicit completion when backspacing past start",
      {
        doc: "foo.o",
        sources: [from("one two")],
      },
    ...
    run.test(
      "stops explicit completions for non-matching input",
      { sources: [from("one")] },
      async (view, sync) => {
        startCompletion(view);
        await sync(options, "one");
    ...
    run.test(
      "resets selection after refinement",
      {
    ...
    run.test(
      "calls sources again when necessary",
      { sources: [tagged(true)] },
      async (view, sync) => {
        type(view, "t");
    ...
    run.test(
      "adjust completions when changes happen during query",
      {
    ...
    run.test(
      "doesn't cancel completions when deleting before they finish",
      {
    ...
    run.test(
      "preserves the dialog on irrelevant changes",
      {
        sources: [from("one two")],
        doc: "woo o",
      },
      async (view, sync) => {
        startCompletion(view);
        await sync(options, "one");
        const dialog = view.dom.querySelector(".cm-tooltip");
        ist(dialog);
    ...
    run.test(
      "replaces entire selected ranges",
      {
    ...
    run.test(
      "complete from list",
      { sources: [once(completeFromList(["one", "two", "three"]))], doc: "t" },
      async (view, sync) => {
        startCompletion(view);
    ...
    run.test(
      "complete from nonalphabetic list",
      {
    ...
    run.test(
      "supports unfitered completions",
      {
        sources: [
          completeFromList(["one", "two"]),
          (cx) => ({ from: cx.pos, options: [{ label: "ok" }, { label: "hah" }], filter: false }),
        ],
        doc: "o",
      },
      async (view, sync) => {
    ...
    run.test(
      "will complete for multiple cursors",
      {
        sources: [from("okay")],
        doc: "o\no",
    ...
    run.test(
      "will not complete for multiple cursors if prefix doesn't match",
      {
    ...
    run.test(
      "can synchronously update results",
      {
        sources: [
          (cx) => ({
            ...
            from: 0,
            update: (r, from, to) => ({
              options: r.options.filter((o) => o.label.length > 1),
              from: r.from,
            }),
          }),
        ],
    ...
    run.test(
      "preserves completion position when changes happen",
      {
        sources: [from("pow")],
        doc: "\n\n",
        selection: 2,
