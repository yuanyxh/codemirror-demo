...
export const completionPlugin = ViewPlugin.fromClass(class implements PluginValue {
  ...
  update(update: ViewUpdate) {
    ...
    if (this.composing != CompositionState.None) for (const tr of update.transactions) {
  ...
  startUpdate() {
    ...
    const {state} = this.view, cState = state.field(completionState)
  ...
  startQuery(active: ActiveSource) {
    const {state} = this.view, pos = cur(state)
    const context = new CompletionContext(state, pos, active.explicitPos == pos, this.view)
  ...
  accept() {
    ...
    for (let i = 0; i < this.running.length; i++) {
      ...
      const current = this.view.state.field(completionState).active.find(a => a.source == query.active.source)
...
export const commitCharacters = Prec.highest(EditorView.domEventHandlers({
  keydown(event, view) {
    ...
    const result = field.active.find(a => a.source == option.source) as ActiveResult
    const commitChars = option.completion.commitCharacters || result.result.commitCharacters
