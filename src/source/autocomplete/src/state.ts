...
function sortOptions(active: readonly ActiveSource[], state: EditorState) {
  ...
  for (const a of active) if (a.hasResult()) {
    ...
    if (a.result.filter === false) {
      ...
      for (const option of a.result.options) if (match = matcher.match(option.label)) {
        const matched = !option.displayLabel ? match.matched : getMatch ? getMatch(option, match.matched) : []
  ...
  if (sections) {
    ...
    const cmp = (a: CompletionSection, b: CompletionSection) => (a.rank ?? 1e9) - (b.rank ?? 1e9) || (a.name < b.name ? -1 : 1)
  ...
  for (const opt of options.sort((a, b) => (b.score - a.score) || compare(a.completion, b.completion))) {
...
export class CompletionState {
  ...
  update(tr: Transaction) {
    ...
    let active: readonly ActiveSource[] = sources.map(source => {
      const value = this.active.find(s => s.source == source) ||
    ...
    for (const effect of tr.effects) if (effect.is(setSelectedEffect)) open = open && open.setSelected(effect.value, this.id)
...
function sameResults(a: readonly ActiveSource[], b: readonly ActiveSource[]) {
  ...
  for (let iA = 0, iB = 0;;) {
    ...
    const endA = iA == a.length, endB = iB == b.length
  ...
  update(tr: Transaction, conf: Required<CompletionConfig>): ActiveSource {
    ...
    for (const effect of tr.effects) {
      if (effect.is(startCompletionEffect))
        ...
        for (const active of effect.value) if (active.source == value.source) value = active
    }
    return value
  }
...
export class ActiveResult extends ActiveSource {
  ...
  updateFor(tr: Transaction, type: UpdateType) {
    ...
    const from = tr.changes.mapPos(this.from), to = tr.changes.mapPos(this.to, 1)
  ...
  map(mapping: ChangeDesc) {
    ...
    const result = this.result.map ? this.result.map(this.result, mapping) : this.result
...
export function applyCompletion(view: EditorView, option: Option) {
  ...
  const result = view.state.field(completionState).active.find(a => a.source == option.source)
