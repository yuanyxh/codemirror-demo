...
function collectWords(doc: Text, cache: WeakMap<Text, readonly Completion[]>, wordRE: RegExp,
  ...
  const result: Completion[] = [], seen: {[word: string]: boolean} = Object.create(null)
  if (doc.children) {
    let pos = 0
...
export const completeAnyWord: CompletionSource = context => {
  const wordChars = context.state.languageDataAt<string>("wordChars", context.pos).join("")
  const re = wordRE(wordChars)
  const token = context.matchBefore(mapRE(re, s => s + "$"))
  if (!token && !context.explicit) return null
  const from = token ? token.from : context.pos
  const options = collectWords(context.state.doc, wordCache(wordChars), re, C.Range, from)
