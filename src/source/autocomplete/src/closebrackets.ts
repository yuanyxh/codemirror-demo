...
const bracketState = StateField.define<RangeSet<typeof closedBracket>>({
  ...
  update(value, tr) {
    ...
    for (const effect of tr.effects) if (effect.is(closeBracketEffect))
  ...
  let dont = null, changes = state.changeByRange(range => {
    ...
    if (next == token) {
      if (nodeStart(state, pos)) {
        ...
        const isTriple = allowTriple && state.sliceDoc(pos, pos + token.length * 3) == token + token + token
...
function probablyInString(state: EditorState, pos: number, quoteToken: string, prefixes: readonly string[]) {
  ...
  for (let i = 0; i < 5; i++) {
    const start = state.sliceDoc(node.from, Math.min(node.to, node.from + quoteToken.length + maxPrefix))
