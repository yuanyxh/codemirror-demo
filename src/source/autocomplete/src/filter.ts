...
export class FuzzyMatcher {
  ...
  constructor(readonly pattern: string) {
    for (let p = 0; p < pattern.length;) {
      const char = codePointAt(pattern, p), size = codePointSize(char)
      this.chars.push(char)
      const part = pattern.slice(p, p + size), upper = part.toUpperCase()
  ...
  match(word: string): {score: number, matched: readonly number[]} | null {
    ...
    // at the start
    if (chars.length == 1) {
      const first = codePointAt(word, 0), firstSize = codePointSize(first)
