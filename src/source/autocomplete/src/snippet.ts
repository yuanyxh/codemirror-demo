...
class Snippet {
  ...
  instantiate(state: EditorState, pos: number) {
    const text = [], lineStart = [pos]
    const lineObj = state.doc.lineAt(pos), baseIndent = /^\s*/.exec(lineObj.text)![0]
  ...
  static parse(template: string) {
    ...
    for (let line of template.split(/\r\n?|\n/)) {
      ...
      line = line.replace(/\\([{}])/g, (_, brace, index) => {
        for (const pos of positions) if (pos.line == lines.length && pos.from > index) {
          pos.from--
          pos.to--
        }
        return brace
...
function moveField(dir: 1 | -1): StateCommand {
  return ({state, dispatch}) => {
    ...
    const next = active.active + dir, last = dir > 0 && !active.ranges.some(r => r.field == next + dir)
