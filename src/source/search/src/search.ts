...
export class SearchQuery {
  ...
  getCursor(state: EditorState | Text, from: number = 0, to?: number): Iterator<{from: number, to: number}> {
    const st = (state as any).doc ? state as EditorState : EditorState.create({doc: state as Text})
...
class StringQuery extends QueryType<SearchResult> {
  ...
  nextMatch(state: EditorState, curFrom: number, curTo: number) {
    ...
    if (cursor.done) {
      const end = Math.min(state.doc.length, curFrom + this.spec.unquoted.length)
  ...
  private prevMatchInRange(state: EditorState, from: number, to: number) {
    for (let pos = to;;) {
      const start = Math.max(from, pos - FindPrev.ChunkSize - this.spec.unquoted.length)
  ...
  matchAll(state: EditorState, limit: number) {
    const cursor = stringCursor(this.spec, state, 0, state.doc.length), ranges = []
  ...
  highlight(state: EditorState, from: number, to: number, add: (from: number, to: number) => void) {
    const cursor = stringCursor(this.spec, state, Math.max(0, from - this.spec.unquoted.length),
...
class RegExpQuery extends QueryType<RegExpResult> {
  ...
  matchAll(state: EditorState, limit: number) {
    const cursor = regexpCursor(this.spec, state, 0, state.doc.length), ranges = []
    while (!cursor.next().done) {
      if (ranges.length >= limit) return null
      ranges.push(cursor.value)
    }
  ...
  highlight(state: EditorState, from: number, to: number, add: (from: number, to: number) => void) {
    const cursor = regexpCursor(this.spec, state, Math.max(0, from - RegExp.HighlightMargin),
...
export const replaceNext = searchCommand((view, {query}) => {
  ...
  if (next) {
    const off = changes.length == 0 || changes[0].from >= next.to ? 0 : next.to - next.from - replacement!.length
...
function announceMatch(view: EditorView, {from, to}: {from: number, to: number}) {
  const line = view.state.doc.lineAt(from), lineEnd = view.state.doc.lineAt(to).to
  const start = Math.max(line.from, from - AnnounceMargin), end = Math.min(lineEnd, to + AnnounceMargin)
