...
class Value extends RangeValue {
  ...
  static names(v: readonly Value[]): string {
    const result = []
    for (const val of v) if (val.name || val.point) result.push(val.name || "POINT")
    return result.join("/")
  }
}
...
function changeSet(changes: [number, number, number][]) {
  const collect: any[] = []
  for (const [from, to, len] of changes) {
...
describe("RangeSet", () => {
  ...
  describe("update", () => {
    it("can add ranges", () => {
      const set = set0().update({add: [mk(4000, {pos: 4000})]})
      ist(set.size, 5001)
    ...
    it("can add a large amount of ranges", () => {
      ...
      const set = set0().update({add: ranges})
      ist(set.size, 2000 + set0().size)
      checkSet(set)
    })

    it("can filter ranges", () => {
      const set = set0().update({filter: from => from >= 2500})
      ist(set.size, 2500)
    ...
    it("can filter all over", () => {
      const set = set0().update({filter: from => (from % 200) >= 100})
      ist(set.size, 2500)
      checkSet(set)
    })

    it("collapses the chunks when removing almost all ranges", () => {
      const set = set0().update({filter: from => from == 500 || from == 501})
      ist(set.size, 2)
      ist(set.chunk.length, 1)
    })
  ...
  describe("map", () => {
    function test(positions: Range<Value>[], changes: [number, number, number][], newPositions: (number | [number, number])[]) {
      const set = mkSet(positions)
      const mapped = set.map(changeSet(changes))
      const out: string[] = []
    ...
    it("can collapse chunks", () => {
      const smaller = set0().map(changeSet([[30, 4500, 0]]))
      ist(smaller.chunk.length, set0().chunk.length, "<")
      const empty = smaller.map(changeSet([[0, 1000, 0]]))
  ...
  describe("compare", () => {
    function test(ranges: RangeSet<Value> | Range<Value>[], update: any, changes: number[]) {
      const set = Array.isArray(ranges) ? mkSet(ranges) : ranges
    ...
    it("ignores changes in points", () => {
      const ranges = [mk(3, 997, {point: true})]
      for (let i = 0; i < 1000; i += 2) ranges.push(mk(i, i + 1, "a"))
      const set = mkSet(ranges.sort(cmp))
      test(set, {
        changes: [[300, 500, 100]]
      }, [])
    })
  ...
  describe("spans", () => {
    ...
    it("can retrieve a limited range", () => {
      const decos = [mk(0, 200, "wide")]
      for (let i = 0; i < 100; i++) decos.push(mk(i * 2, i * 2 + 2, "span" + i))
      const set = mkSet(decos), start = 20, end = start + 6
      let expected = ""
    ...
    it("reads from multiple sets at once", () => {
      const one = mkSet([mk(2, 3, "x"), mk(5, 10, "y"), mk(10, 12, "z")])
      const two = mkSet([mk(0, 6, "a"), mk(10, 12, "b")])
      test([one, two], 0, 12, "2=a 1=x/a 2=a 1=y/a 4=y 2=z/b")
    })

    it("orders active ranges by origin set", () => {
      const one = mkSet([mk(2, 10, "a"), mk(20, 30, "a")])
      const two = mkSet([mk(3, 4, "b"), mk(8, 12, "b"), mk(18, 22, "b")])
      const three = mkSet([mk(0, 25, "c")])
  ...
  describe("between", () => {
    ...
    it("returns ranges in a zero-length set", () => {
      const set = RangeSet.of([mk(0, 0)]), found: number[] = []
