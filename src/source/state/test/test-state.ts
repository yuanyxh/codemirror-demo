...
describe("EditorState", () => {
  ...
  it("can apply changes", () => {
    const state = EditorState.create({doc: "hello"})
    const transaction = state.update({changes: [{from: 2, to: 4, insert: "w"}, {from: 5, insert: "!"}]})
  ...
  it("maps selection through changes", () => {
    ...
    const newState = state.update(state.replaceSelection("Q")).state
  ...
  it("can store annotations on transactions", () => {
    const tr = EditorState.create({doc: "foo"}).update({annotations: someAnnotation.of(55)})
    ist(tr.annotation(someAnnotation), 55)
  })

  it("throws when a change's bounds are invalid", () => {
    const state = EditorState.create({doc: "1234"})
  ...
  it("stores and updates tab size", () => {
    const deflt = EditorState.create({}), two = EditorState.create({extensions: [EditorState.tabSize.of(2)]})
    ist(deflt.tabSize, 4)
    ist(two.tabSize, 2)
    const updated = deflt.update({effects: StateEffect.reconfigure.of(EditorState.tabSize.of(8))}).state
    ist(updated.tabSize, 8)
  })

  it("stores and updates the line separator", () => {
    const deflt = EditorState.create({}), crlf = EditorState.create({extensions: [EditorState.lineSeparator.of("\r\n")]})
    ist(deflt.facet(EditorState.lineSeparator), null)
    ist(deflt.toText("a\nb").lines, 2)
    ist(crlf.facet(EditorState.lineSeparator), "\r\n")
    ist(crlf.toText("a\nb").lines, 1)
    const updated = crlf.update({effects: StateEffect.reconfigure.of(EditorState.lineSeparator.of("\n"))}).state
    ist(updated.facet(EditorState.lineSeparator), "\n")
  })

  it("stores and updates fields", () => {
    const field1 = StateField.define<number>({create: () => 0, update: val => val + 1})
    const field2 = StateField.define<number>({create: state => state.field(field1) + 10, update: val => val})
    const state = EditorState.create({extensions: [field1, field2]})
  ...
  it("allows fields to have an initializer", () => {
    const field = StateField.define<number>({create: () => 0, update: val => val + 1})
    const state = EditorState.create({extensions: field.init(() => 10)})
    ist(state.field(field), 10)
    ist(state.update({}).state.field(field), 11)
  })

  it("can be serialized to JSON", () => {
    const field = StateField.define<{n: number}>({
      create() { return {n: 0} },
    ...
    const state = EditorState.create({extensions: field}).update({}).state
    const json = state.toJSON(fields)
    ist(JSON.stringify(json.f), '{"number":1}')
    const state2 = EditorState.fromJSON(json, {}, fields)
    ist(JSON.stringify(state2.field(field)), '{"n":1}')
  })

  it("can preserve fields across reconfiguration", () => {
    const field = StateField.define({create: () => 0, update: val => val + 1})
    const start = EditorState.create({extensions: [field]}).update({}).state
  ...
  it("can replace extension groups", () => {
    const comp = new Compartment, f = Facet.define<number>(), content = f.of(10)
    const state = EditorState.create({extensions: [comp.of(content), f.of(20)]})
    ist(comp.get(state), content)
    ist(state.facet(f).join(), "10,20")
    const content2 = [f.of(1), f.of(2)]
    const state2 = state.update({effects: comp.reconfigure(content2)}).state
    ist(comp.get(state2), content2)
    ist(state2.facet(f).join(), "1,2,20")
    const state3 = state2.update({effects: comp.reconfigure(f.of(3))}).state
    ist(state3.facet(f).join(), "3,20")
  })

  it("raises an error on duplicate extension groups", () => {
    const comp = new Compartment, f = Facet.define<number>()
  ...
  it("preserves compartments on reconfigure", () => {
    const comp = new Compartment, f = Facet.define<number>(), init = comp.of(f.of(10))
  ...
  it("allows facets computed from fields", () => {
    const field = StateField.define({create: () => [0], update: (v, tr) => tr.docChanged ? [tr.state.doc.length] : v})
    const facet = Facet.define<number>()
    const state = EditorState.create({
      extensions: [field, facet.compute([field], state => state.field(field)[0]), facet.of(1)]
    })
    ist(state.facet(facet).join(), "0,1")
    const state2 = state.update({}).state
    ist(state2.facet(facet), state.facet(facet))
    const state3 = state.update({changes: {insert: "hi", from: 0}}).state
    ist(state3.facet(facet).join(), "2,1")
  })

  it("blocks multiple selections when not allowed", () => {
    const cursors = EditorSelection.create([EditorSelection.cursor(0), EditorSelection.cursor(1)])
    const state = EditorState.create({
      selection: cursors,
      doc: "123"
    })
  ...
  describe("changeFilter", () => {
    it("can cancel changes", () => {
      ...
      const tr1 = state.update({changes: {from: 3, insert: " three"}, selection: {anchor: 13}})
      ist(tr1.state.doc.toString(), "one two")
      ist(tr1.state.selection.main.head, 7)
      const tr2 = state.update({changes: {from: 4, to: 7, insert: "2"}})
    ...
    it("can be turned off", () => {
      const state = EditorState.create({extensions: [EditorState.changeFilter.of(() => false)]})
  ...
  describe("transactionExtender", () => {
    it("can add annotations", () => {
      const ann = Annotation.define<number>()
      const state = EditorState.create({
      ...
      const trNoFilter = state.update({changes: {from: 0, insert: "!"}, filter: false})
      ist(trNoFilter.annotation(ann), 100)
    })

    it("allows multipe extenders to take effect", () => {
      const eff = StateEffect.define<number>()
      const state = EditorState.create({
        extensions: [
