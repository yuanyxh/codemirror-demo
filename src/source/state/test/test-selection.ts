...
describe("EditorSelection", () => {
  it("stores ranges with a primary range", () => {
    const sel = EditorSelection.create([EditorSelection.range(0, 1), EditorSelection.range(3, 2), EditorSelection.range(4, 5)], 1)
    ist(sel.main.from, 2)
    ist(sel.main.to, 3)
  ...
  it("preserves the direction of the last range when merging ranges", () => {
    const sel = EditorSelection.create([EditorSelection.range(0, 2), EditorSelection.range(10, 1)])
