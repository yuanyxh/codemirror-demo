...
describe("ChangeDesc", () => {
  describe("composition", () => {
    function comp(...specs: string[]) {
      const result = specs.pop(), sets = specs.map(mk)
  ...
  describe("mapPos", () => {
    function map(spec: string, ...cases: [number, number | null, (number | string)?][]) {
      const set = mk(spec)
      for (const [from, to, opt] of cases) {
        const assoc = typeof opt == "number" ? opt : -1
        const mode = ({D: MapMode.TrackDel, A: MapMode.TrackAfter, B: MapMode.TrackBefore} as any)[opt as any]
...
describe("ChangeSet", () => {
  ...
  const doc10 = Text.of(["0123456789"])
  ...
  it("can apply mapped sets", () => {
    const set0 = ChangeSet.of([{insert: "hi", from: 5}, {from: 8, to: 10}], 10)
    const set1 = ChangeSet.of([{insert: "ok", from: 10}, {from: 6, to: 7}], 10)
  ...
  it("can be iterated", () => {
    const set = ChangeSet.of([{insert: "ok", from: 4}, {from: 6, to: 8}], 10)
    let result: any[] = []
  ...
  it("mapping before produces the same result as mapping the other after", () => {
    for (let i = 0, total = 100; i < total; i++) {
      const size = r(20), count = Math.floor(i / (total / 10)) + 1
      const a = rChanges(size, count), b = rChanges(size, count)
      try {
        const setA = ChangeSet.of(a, size), setB = ChangeSet.of(b, size)
        const setA1 = setA.map(setB, true), setB1 = setB.map(setA, false)
        const doc = Text.of([rStr(size)])
        const setAB = setA.compose(setB1), setBA = setB.compose(setA1)
  ...
  it("mapping still converges when mapping through multiple changes", () => {
    for (let i = 0, total = 100; i < total; i++) {
      const size = r(20), count = Math.floor(i / (total / 10)) + 1
      const a = ChangeSet.of(rChanges(size, count), size)
      const b = ChangeSet.of(rChanges(a.newLength, count), a.newLength)
      const c = ChangeSet.of(rChanges(size, count), size)
      const c$a = c.map(a), c$ab = c$a.map(b)
      const a$c = a.map(c, true), b$ca = b.map(c$a, true)
      const doc = Text.of([rStr(size)])
  ...
  it("compose produces the same result as individual changes", () => {
    for (let i = 0; i < 100; i++) {
      const size = r(20), doc = Text.of([rStr(size)])
      const a = ChangeSet.of(rChanges(size, r(5) + 1), size)
      const b = ChangeSet.of(rChanges(a.newLength, r(6)), a.newLength)
  ...
  it("composing is associative", () => {
    for (let i = 0; i < 100; i++) {
      ...
      const a = ChangeSet.of(rChanges(size, r(5) + 1), size)
      const b = ChangeSet.of(rChanges(a.newLength, r(6)), a.newLength)
      const c = ChangeSet.of(rChanges(b.newLength, r(5) + 1), b.newLength)
      const left = a.compose(b).compose(c), right = a.compose(b.compose(c))
    ...
    for (let i = 0; i < 50; i++) {
      ...
      try {
        for (let j = 0; j < 50; j++) {
          ...
          const {from, to = from, insert = ""} = change
        ...
        const composed = all.reduce((a, b) => a.compose(b), ChangeSet.of([], doc10.length))
  ...
  it("can be serialized to JSON", () => {
    for (let i = 0; i < 100; i++) {
      const size = r(20) + 1, set = ChangeSet.of(rChanges(size, r(4)), size)
