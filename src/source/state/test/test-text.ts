...
describe("Text", () => {
  ...
  it("creates a balanced tree when loading a document", () => {
    const doc = Text.of(new Array(2000).fill(line)), d = depth(doc)
    ist(d, 2, "<=")
  })

  it("rebalances on insert", () => {
    let doc = doc0
    const insert = "abc".repeat(200), at = Math.floor(doc.length / 2)
  ...
  it("collapses on delete", () => {
    const doc = doc0.replace(10, text0.length - 10, Text.empty)
    ist(depth(doc), 1)
    ist(doc.length, 20)
  ...
  it("can insert on node boundaries", () => {
    const doc = doc0, pos = doc.children![0].length
  ...
  it("properly maintains content during editing", () => {
    ...
    for (let i = 0; i < 200; i++) {
      const insPos = Math.floor(Math.random() * doc.length)
      const insChar = String.fromCharCode("A".charCodeAt(0) + Math.floor(Math.random() * 26))
      str = str.slice(0, insPos) + insChar + str.slice(insPos)
      doc = doc.replace(insPos, insPos, Text.of([insChar]))
      const delFrom = Math.floor(Math.random() * doc.length)
      const delTo = Math.min(doc.length, delFrom + Math.floor(Math.random() * 20))
  ...
  it("can be compared", () => {
    const doc = doc0, doc2 = Text.of(lines)
    ist(doc.eq(doc))
    ist(doc.eq(doc2))
    ist(doc2.eq(doc))
  ...
  it("is iterable", () => {
    for (let iter = doc0.iter(), build = "";;) {
      const {value, lineBreak, done} = iter.next()
      if (done) {
        ist(build, text0)
        break
      }
  ...
  it("allows negative skip values in iteration", () => {
    const iter = Text.of(["one", "two", "three", "four"]).iter()
  ...
  it("can retrieve pieces of text", () => {
    for (let i = 0; i < 500; i++) {
      const from = Math.floor(Math.random() * (doc0.length - 1))
      const to = Math.random() < .5 ? from + 2 : from + Math.floor(Math.random() * (doc0.length - 1 - from)) + 1
