...
class FacetProvider<Input> {
  ...
  dynamicSlot(addresses: {[id: number]: number}): DynamicSlot {
    ...
    const id = this.id, idx = addresses[id] >> 1, multi = this.type == Provider.Multi
...
function dynamicFacetSlot<Input, Output>(
  ...
  return {
    ...
    reconfigure(state, oldState) {
      ...
      const oldProviders = oldState.config.facets[facet.id], oldValue = oldState.facet(facet)
...
export class StateField<Value> {
  ...
  static define<Value>(config: StateFieldSpec<Value>): StateField<Value> {
    const field = new StateField<Value>(nextID++, config.create, config.update, config.compare || ((a, b) => a === b), config)
...
export class Configuration {
  ...
  static resolve(base: Extension, compartments: Map<Compartment, Extension>, oldState?: EditorState) {
    ...
    for (const id in facets) {
      const providers = facets[id], facet = providers[0].facet
