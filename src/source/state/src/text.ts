...
export abstract class Text implements Iterable<string> {
  ...
  eq(other: Text): boolean {
    ...
    const start = this.scanIdentical(other, 1), end = this.length - this.scanIdentical(other, -1)
    const a = new RawTextCursor(this), b = new RawTextCursor(other)
...
class TextLeaf extends Text {
  ...
  lineInner(target: number, isLine: boolean, line: number, offset: number): Line {
    for (let i = 0;; i++) {
      const string = this.text[i], end = offset + string.length
  ...
  decompose(from: number, to: number, target: Text[], open: Open) {
    const text = from <= 0 && to >= this.length ? this
  ...
  replace(from: number, to: number, text: Text): Text {
    ...
    ;[from, to] = clip(this, from, to)
    const lines = appendText(this.text, appendText(text.text, sliceText(this.text, 0, from)), to)
  ...
  sliceString(from: number, to = this.length, lineSep = "\n") {
    ...
    for (let pos = 0, i = 0; pos <= to && i < this.text.length; i++) {
      const line = this.text[i], end = pos + line.length
      if (pos > from && i)
        result += lineSep
      if (from < end && to > pos)
...
class TextNode extends Text {
  ...
  lineInner(target: number, isLine: boolean, line: number, offset: number): Line {
    for (let i = 0;; i++) {
      const child = this.children[i], end = offset + child.length, endLine = line + child.lines - 1
  ...
  decompose(from: number, to: number, target: Text[], open: Open) {
    for (let i = 0, pos = 0; pos <= to && i < this.children.length; i++) {
      ...
      if (from <= end && to >= pos) {
        const childOpen = open & ((pos <= from ? Open.From : 0) | (end >= to ? Open.To : 0))
  ...
  scanIdentical(other: Text, dir: -1 | 1): number {
    ...
    for (;; iA += dir, iB += dir) {
      ...
      const chA = this.children[iA], chB = other.children[iB]
  ...
  static from(children: Text[], length: number = children.reduce((l, ch) => l + ch.length + 1, -1)): Text {
    ...
    const chunk = Math.max(Tree.Branch, lines >> Tree.BranchShift), maxChunk = chunk << 1, minChunk = chunk >> 1
...
class RawTextCursor implements TextIterator {
  ...
  nextInner(skip: number, dir: 1 | -1): this {
    ...
    for (;;) {
      ...
      const top = this.nodes[last], offsetValue = this.offsets[last], offset = offsetValue >> 1
      const size = top instanceof TextLeaf ? top.text.length : top.children!.length
