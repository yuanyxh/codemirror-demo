...
export class ChangeSet extends ChangeDesc {
  ...
  invert(doc: Text) {
    const sections = this.sections.slice(), inserted = []
  ...
  filter(ranges: readonly number[]) {
    const resultSections: number[] = [], resultInserted: Text[] = [], filteredSections: number[] = []
  ...
  toJSON(): any {
    ...
    for (let i = 0; i < this.sections.length; i += 2) {
      const len = this.sections[i], ins = this.sections[i + 1]
  ...
  static of(changes: ChangeSpec, length: number, lineSep?: string): ChangeSet {
    ...
    function process(spec: ChangeSpec) {
      if (Array.isArray(spec)) {
        ...
        const {from, to = from, insert} = spec as {from: number, to?: number, insert?: string | Text}
        if (from > to || from < 0 || to > length)
          throw new RangeError(`Invalid change range ${from} to ${to} (in doc of length ${length})`)
        const insText = !insert ? Text.empty : typeof insert == "string" ? Text.of(insert.split(lineSep || DefaultSplit)) : insert
...
function mapSet(setA: ChangeDesc, setB: ChangeDesc, before: boolean, mkSet = false): ChangeSet | ChangeDesc {
  ...
  const sections: number[] = [], insert: Text[] | null = mkSet ? [] : null
...
class SectionIter {
  ...
  get text() {
    const {inserted} = this.set as ChangeSet, index = (this.i - 2) >> 1
