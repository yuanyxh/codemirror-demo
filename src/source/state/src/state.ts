...
export class EditorState {
  ...
  applyTransaction(tr: Transaction) {
    ...
    if (!conf) {
      ...
      const intermediateState = new EditorState(conf, this.doc, this.selection, conf.dynamicSlots.map(() => null),
  ...
  toJSON(fields?: {[prop: string]: StateField<any>}): any {
    ...
    if (fields) for (const prop in fields) {
  ...
  static fromJSON(json: any, config: EditorStateConfig = {}, fields?: {[prop: string]: StateField<any>}): EditorState {
    ...
    if (fields) for (const prop in fields) {
      if (Object.prototype.hasOwnProperty.call(json, prop)) {
        const field = fields[prop], value = json[prop]
  ...
  static create(config: EditorStateConfig = {}): EditorState {
    ...
    const doc = config.doc instanceof Text ? config.doc
